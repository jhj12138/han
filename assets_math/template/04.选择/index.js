...
function selectItem(event) {
  var allSelect = document.getElementsByClassName('select-item')
  for(var i = 0; i < allSelect.length; i++) {
    allSelect[i].style.background = '#fff'
  }
  currentSelectId = event.target.dataset.select;
  console.log(currentSelectId)
  var currentSelect_item = document.getElementById(event.target.id)
  currentSelect_item.style.background = select_Color
}
/** 
 * 
*/
function decideSelect() {
  if(rigth_select == currentSelectId) {
    return true
  }else {
    return false
  }
...
function calcData(userObj, rightObj) {
  ...
  if(rightObj.sort) {
    if(userObj.data.toString() == rightObj.data.toString() && userObj.type == rightObj.type && userObj.result == right_result && decideSelect()) {
      alert('通过')
    }else {
      alert('错误')
    }
  }else {
    if(userObj.data.sort().toString() == rightObj.data.sort().toString() && userObj.type == rightObj.type && userObj.result == right_result && decideSelect()) {
      alert('通过')
    }else {
      alert('错误')
    }
