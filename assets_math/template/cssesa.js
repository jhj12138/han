...
function getDrag(domName,checkBoxs,mobileClass) {
  var checkBox = document.getElementsByClassName(checkBoxs) //获取上面需要填写的div
            ...
            if(count === 0){
              ...
              if (checkBox[k] != undefined){
                if (!checkBox[k].children.length) {
                  ...
                  if(domName == 'u6-zs1') {
                    clonedNode.style.cssText = "background-image: url('./images/commen/unitSix/zsOne"+getIndex(e.target)+".png');background-repeat: no-repeat;background-size: cover;width: 47%;height: 32%;position: absolute;left: 50%; top: 50%; transform: translate(-50%, -50%)"
                  } 
                  ...
                  for (var j = 0 ; j < mobileBox.length; j++) {
                    if (checkBox[k].firstChild.style.backgroundImage.replace(/[^0-9]/ig,"")-1 == j) {
                      console.log(checkBox[k].firstChild)
                ...
                for (var i = 0; i <locat.length; i++) {
                  if(x1 >= locat[i].x1 && y1 >= locat[i].y1 && x1 <= locat[i].x2 && y1 <=locat[i].y2) {
                    if(checkBox[i].firstChild) { //目标box有东西进行替换
                      ...
                      clonedNode.classList.remove('mobile_box')
                      if(domName == 'u6-zs1') {
                        clonedNode.style.cssText = "background-image: url('./images/commen/unitSix/zsOne"+getPicInTopBox(removeBox.target)+".png');background-repeat: no-repeat;background-size: cover;width: 47%;height: 32%;position: absolute;left: 50%; top: 50%; transform: translate(-50%, -50%)"
                      } 
